...
train_dataset = FacesDataset(train_unlabeled, mode='train_unlabeled', augmentations=False)
...
path = os.getcwd()
fn = path + '/experiments/' + '/0_UNLABELED'
fplot = fn + '/plots'
try:
    os.mkdir(fn)
    os.mkdir(fplot)
...
val_dataset = FacesDataset(val_files, mode='val_unlabeled', augmentations=False)
...
val_loss1, val_acc1, model_s = single(torchvision.models.resnet50, train_dl, val_dl, fn, lr=1e-5, n_epochs=10,
                                      epoch_step=1, num_classes=4)

torch.save(model_s.state_dict(), 'unlabeled_model')
